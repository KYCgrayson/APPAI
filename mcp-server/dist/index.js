...
// --- Tools ---
server.tool("start-auth", "Start device authorization flow to get an AppAI API key. Returns a verification URL the user must open in their browser to sign in with Google.", {}, async () => {
    const result = await apiRequest("/api/v1/auth/device", { method: "POST" });
    return {
        content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
    };
});
server.tool("poll-auth", "Poll for authentication completion after start-auth. Returns the API key when the user has authorized.", {
...
server.tool("list-presets", "List available page preset templates (app landing, SaaS, profile, link-in-bio, portfolio, event). Each preset includes recommended sections and a live preview URL.", {}, async () => {
    const result = await apiRequest("/api/v1/presets");
    return {
        content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
    };
});
server.tool("list-sections", "List all available section types that can be used when building a page (hero, features, pricing, FAQ, etc.) with their data schemas.", {}, async () => {
    const result = await apiRequest("/api/v1/sections");
    return {
        content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
    };
});
server.tool("create-page", "Create a new hosted page on AppAI. Includes landing page, privacy policy, and terms of service. The page is instantly live at appai.info/p/{slug}.", {
...
server.tool("list-pages", "List all pages owned by the authenticated user.", {
...
server.tool("get-page", "Get details of a specific page by slug.", {
    api_key: z.string().describe("AppAI API key"),
...
server.tool("update-page", "Update an existing page (PATCH for partial update with deep merge, PUT for full replace).", {
    api_key: z.string().describe("AppAI API key"),
    slug: z.string().describe("Page slug"),
...
server.tool("delete-page", "Delete a page by slug. Optionally delete only a specific locale variant.", {
    api_key: z.string().describe("AppAI API key"),
    slug: z.string().describe("Page slug"),
...
server.tool("set-default-locale", "Change the default locale for a page.", {
    api_key: z.string().describe("AppAI API key"),
    slug: z.string().describe("Page slug"),
...
// --- Prompts ---
server.prompt("create-app-page", { app_name: z.string().optional().describe("Name of the app") }, ({ app_name }) => ({
    messages: [
        {
            role: "user",
            content: {
...
server.prompt("setup-privacy-policy", { app_name: z.string().optional().describe("Name of the app") }, ({ app_name }) => ({
    messages: [
        {
            role: "user",
            content: {
