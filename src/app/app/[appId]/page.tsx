...
export default async function UniversalAppLauncher({ params }: { params: Promise<{ appId: string }> }) {
  const parsedAppId = universalAppIdSchema.safeParse((await params).appId);
  if (!parsedAppId.success) return <Unavailable message="找不到這個應用程式。" />;

  const requestHeaders = await headers();
  const returnPath = safeInternalPath(requestHeaders.get("x-appai-request-path"), `/app/${parsedAppId.data}`);
  let context;
  try {
