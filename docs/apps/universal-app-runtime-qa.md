...
External submission creates a `PENDING` release awaiting platform review. An
AppAI administrator, not the submitting agent, starts the managed validation,
build, database, migration, deployment, health, and activation pipeline. The
agent polls release status and never provides runtime URLs, database or provider
credentials, raw SQL, or platform infrastructure settings.
...
| G1 — Artifact | Verify a credential-free public `https://github.com/{owner}/{repo}` source and exact 40-character commit SHA; reject other Git/HTTPS hosts; validate the lockfile-strict install command plus safe build/start/migration commands; require `package.json` `test` and `typecheck` scripts; AppAI sandbox installs from the lockfile, runs those scripts and the declared build command, and binds the computed immutable digest. | Provider evidence ties repo ID + commit + manifest + computed digest to the build; test, typecheck, and build commands exit 0; health, callback, and app routes exist. A reviewer separately records any dependency-audit result. | Private, credentialed, mutable, or foreign-host source; missing test/typecheck script; digest mismatch; generated-file drift; test/type/build failure; reviewer-found unsafe dependency; or build secret exposure. |
| G2 — Database | AppAI provisions an app-stable schema and migration owner plus a deployment-scoped runtime role; rehearse the declared migration command with rollback; verify schema diff; test runtime role CRUD and DDL/cross-app denial. | Rehearsal/apply exit 0; post-apply diff empty; runtime CRUD succeeds and CREATE/ALTER/DROP/cross-schema access fails. | Runtime credential has migration, platform DB, or cross-app access. |
...
| G7 — Cutover/rollback | Provider deployment is healthy and protected before activation. With approved + active production deployment, launcher redirects. Invalid release, inactive deployment, or suspended instance fails closed. Roll back atomically to the last healthy immutable release. | State matrix passes; provider project/deployment ownership, health timestamp, and active pointer are recorded; rollback restores health without writing to both schemas. | Public/unverified runtime, split-brain writes, catch-all fallback, destructive migration, or rollback requires source edits. |
...
| Approved release + active production deployment | Issue one-time launch code and redirect to the isolated platform-managed `https://{appId}.appai.info` runtime. |
